...
export default function CalendarTab() {
  ...
  const dayColor = (key: string) => {
    const src = blocked.find(r => { const s = new Date(r.check_in); const e = new Date(r.check_out); return new Date(key) >= s && new Date(key) < e })?.source
  ...
  async function blockDate() {
    if (!adding) return
    await fetch('/api/admin/blocked-dates', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ date:adding, notes:note }) })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3 text-xs">
        {[['#FF5A5F','Airbnb'],['#3B82F6','Booking.com'],['#22C55E','Direct booking'],['#9CA3AF','Owner block']].map(([c,l]) => (
          <span key={l} className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm" style={{ background:c }} />{l}</span>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 gap-1 text-center">
        {['Mo','Tu','We','Th','Fr','Sa','Su'].map(d => <div key={d} className="text-white/30 text-xs py-1">{d}</div>)}
