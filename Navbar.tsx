...
export async function POST(req: NextRequest) {
  try {
    const { checkIn, checkOut, numGuests, nightlyRate, total,
            guestName, guestEmail, guestPhone, specialRequests } = await req.json()
    ...
    const reservation: ReservationInsert = {
      ...
      nightly_rate: nightlyRate, total_amount: total, currency: 'EUR',
      status: 'pending', payment_status: 'pending', booking_source: 'direct',
    ...
    const session = await createCheckoutSession({
      reservationId: created.id, nightlyRate,
      numNights: Math.round(total / nightlyRate),
      totalAmount: total, checkIn, checkOut, guestName, guestEmail,
    })
